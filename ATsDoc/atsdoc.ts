...
namespace atsdoc {
    ...
    function processProgram(): ReturnCode {
        ...
        if (tscCommandLine.options.project) {
            ...
            const fileOrDirectory: string = nodepath.normalize(tscCommandLine.options.project);
    ...
    function collectNodeData(node: ts.Node, atsNode: IATsDocNode): void {
        ...
        if (anyNode.kind === ts.SyntaxKind.VariableDeclaration) {
            ...
            /* tslint:disable */
            if (ps.indexOf("let") !== -1) atsNode.atsNodeFlags = atsNode.atsNodeFlags | ATsDocNodeFlags.let;
            if (ps.indexOf("var") !== -1) atsNode.atsNodeFlags = atsNode.atsNodeFlags | ATsDocNodeFlags.var;
    ...
    function lintts(program: ts.Program): void {
        ...
        // tslint path
        let tslintDir: string = "./node_modules/tslint/";
        ...
        let configPath: string = nodepath.resolve(nodepath.normalize(tslintDir + "/lib/configs/recommended.js"));
